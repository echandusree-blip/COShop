...
import { Users, X, Link as LinkIcon, Check, Flame, Sparkles, ShieldCheck, ShoppingCart, CreditCard, CheckCircle, AlertTriangle, SlidersHorizontal, Search } from 'lucide-react';
...
export default function App() {
  ...
  return <MarketplaceApp setIsAuthenticated={setIsAuthenticated} setCurrentView={setCurrentView} />;
}

function MarketplaceApp({ setIsAuthenticated, setCurrentView }) {
    ...
    <BrowserRouter>
      <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh', fontFamily: 'sans-serif', position: 'relative', overflowX: 'hidden' }}>
        ...
        <nav style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '15px 30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
          ...
          {/* Working Search Bar */}
          <div style={{ display: 'flex', alignItems: 'center', background: '#f1f5f9', padding: '6px 14px', borderRadius: '10px', border: '1px solid #cbd5e1', width: '320px', gap: '10px' }}>
            ...
            <input 
              ...
              style={{ background: 'transparent', border: 'none', outline: 'none', width: '100%', fontSize: '14px', color: '#0f172a' }}
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
            ...
            <button 
              ...
              style={{ backgroundColor: '#ef4444', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px' }}
            >
              Logout
            </button>
          </div>
        ...
        <Routes>
          <Route path="/" element={<MarketplaceHome products={products} searchQuery={searchQuery} cart={cart} setCart={setCart} activeUserPools={activeUserPools} setActiveUserPools={setActiveUserPools} setIsDrawerOpen={setIsDrawerOpen} />} />
...
function MarketplaceHome({ products, searchQuery, cart, setCart, activeUserPools, setActiveUserPools, setIsDrawerOpen }) {
      ...
      <div style={{ flex: '1', padding: '40px 30px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '25px', padding: '20px' }}>
          {filteredProducts.length === 0 ? (
            <p style={{ color: '#64748b', fontSize: '15px' }}>No products found matching "{searchQuery}"</p>
          ) : (
